...
(function() {
  var bindFormKeyup, disableDownloadLink, enableDownloadLink, getTrimmedValue, hideIntroPanel, initDownloadLinks, initHideIntroPanel, initShowHideIntro, initYearFormField, showErrorMessage, showIntroPanel, showSharePanel, showYearForm, updateDownloadLink, validateYearInput, validateYearRange, valueIsInteger, yearFieldFocus;
  ...
  showYearForm = function() {
    return $('#cu_form_wrap').css({
      ...
      return initYearFormField();
    });
  };

  showSharePanel = function() {
    var $sharePanel;
    $sharePanel = $('#cu_form_share');
    if (!$sharePanel.is(":visible")) {
      $('#cu_form_info').hide();
      return $sharePanel.css({
        opacity: .5,
        display: 'block',
        x: 25
      }).transition({
